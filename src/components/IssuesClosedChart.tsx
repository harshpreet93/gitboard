...
interface IssuesClosedChartProps {
  repos: ParsedRepo[];
}
...
export function IssuesClosedChart({ repos }: IssuesClosedChartProps) {
  ...
  useEffect(() => {
    const fetchRepo = (repo: ParsedRepo) => {
      ...
      fetchIssuesClosedOverTime(repo.owner, repo.name)
        .then((data) => {
          setRepoData((prev) => {
            const next = new Map(prev);
            next.set(key, {
  ...
  if (repos.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Weekly Issues Closed</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex h-[300px] items-center justify-center text-muted-foreground">
            Add repositories to see issue resolution activity
          </div>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Weekly Issues Closed (Last 52 Weeks)</CardTitle>
      </CardHeader>
      <CardContent>
        {errors.length > 0 && (
        ...
        {!isLoading && chartData.length > 0 && (
          <div role="img" aria-label={`Line chart showing weekly issues closed over the last 52 weeks for ${repoKeys.join(", ")}`}>
